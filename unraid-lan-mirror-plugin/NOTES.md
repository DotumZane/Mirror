...
- Push to GitHub, update both Unraid servers to `0.6.6`, and try starting remote sync again.
...
- Versioned package build exists at `packages/mirror-0.6.6.txz`.
...
2. On both Unraid servers, update Mirror to `0.6.6`.
...
- Next suggested task: Push local commits to GitHub, update both Unraid servers to `0.6.6`, start the daemon with a remote share selected, then implement master-pushed remote configuration.
