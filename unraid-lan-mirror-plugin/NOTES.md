...
- Push to GitHub, update both Unraid servers to `0.7.1`, and check the Start action output plus Recent Log.
...
- Versioned package build exists at `packages/mirror-0.7.1.txz`.
...
2. On both Unraid servers, update Mirror to `0.7.1`.
...
- Next suggested task: Push local commits to GitHub, update both Unraid servers to `0.7.1`, click Start on the Master, and capture the action output if it still fails.
