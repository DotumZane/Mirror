...
- Push to GitHub, update both Unraid servers to `0.6.7`, and check Recent Log after starting remote sync.
...
- Versioned package build exists at `packages/mirror-0.6.7.txz`.
...
2. On both Unraid servers, update Mirror to `0.6.7`.
...
- Next suggested task: Push local commits to GitHub, update both Unraid servers to `0.6.7`, start the daemon, then inspect Recent Log for remote sync errors.
